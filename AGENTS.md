...
- **Zustand** stores call **service layer** (`services/db.js`, `services/auth.js`, `services/storage.js`). Services talk to Hono API.
...
- **Expo Router** file-based routing in `mobile/app/`. `_layout.tsx` files define navigators (Stack, Tabs).
- **ClerkProvider** in `app/_layout.tsx` with `tokenCache` (SecureStore). Auth guard redirects based on `isSignedIn`.
- **Tab navigator** in `app/(app)/_layout.tsx`: Dashboard, Lagu, Setlist, Jadwal, Pengaturan (5 tabs).
- **Zustand** stores follow the same pattern as web — `useSongStore`, `useSetlistStore`, `useSessionStore`, `useRoleStore`.
...
- **Stack layouts** for nested routes: `songs/_layout.tsx`, `setlists/_layout.tsx`, `sessions/_layout.tsx`.
...
- **Auth flow**: Login → `signIn.create()` → `setActive()` → redirect `/(app)`. Register → `signUp.create()` → OTP → `setActive()`.
- **SecureStore** for Clerk session tokens (not AsyncStorage).
- **Azure/Cloudinary**: `uploadAudioToCloudinary()` uses unsigned preset from browser, delete via server API.
...
- **No push notifications**: `expo-notifications` installed but not configured.
